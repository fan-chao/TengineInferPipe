...
namespace mediapipe {
    ...
    absl::Status TensorrtYolov5TensorsToDetectionsCalculator::ProcessCPU(
        ...
        if (input_tensors.size() == 1) {
            ...
            //infer out box info format is: cent_x, cent_y, width, height, obj_thresh, clas_id, score_c1, score_c2, score_c3
            for (int i = 0; i < num_boxes; ++i) {
                if (*(raw_data + 4) > options_.min_score_thresh()){
                    std::pair<int, float> max_result = argmax(raw_data + 6, num_classes_);
                    float score_val = max_result.second;
                    if (score_val > options_.min_score_thresh()) {
                        float x1 = raw_data[0] - raw_data[2] / 2;
                        float y1 = raw_data[1] - raw_data[3] / 2;
                        float x2 = raw_data[0] + raw_data[2] / 2;
                        float y2 = raw_data[1] + raw_data[3] / 2;

                        x1 = (x1 / img_width_ - input_pads[0]) / ori_width;
                        y1 = (y1 / img_height_ - input_pads[1]) / ori_height;
                        x2 = (x2 / img_width_ - input_pads[0]) / ori_width;
                        y2 = (y2 / img_height_ - input_pads[1]) / ori_height;
                        if (x1 >= 0 && x1 < 1 && x2 >= 0 && x2 < 1 && y1 >= 0 && y1 < 1 && y2 >= 0 && y2 < 1){
                            //printf("x1=%.03f,y1=%.03f, x2=%.03f,y2=%.03f\n", x1, y1, x2, y2);
